class NonSmoker::SavingsGoalsController < ApplicationController
  ...
  before_action :set_savings_goal, only: [:update, :destroy]
  before_action :ensure_quit_smoking_record, only: [:create]
  before_action :ensure_no_active_goal, only: [:create]
  before_action :set_current_savings, only: [:index, :create, :update]
  before_action :set_active_goal, only: [:index, :create, :update]
  before_action :set_achieved_goals, only: [:index, :create, :update]
  before_action :set_progress_data, only: [:index, :create, :update], if: -> { @active_goal }
  ...
  def ensure_quit_smoking_record
    unless current_quit_smoking_record&.active?
